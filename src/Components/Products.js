      ...
      <div className='items-center flex flex-col w-full'>
        <div className='grid w-full grid-cols-1 place-items-center sm:pl-4 sm:pr-4 lg:pl-4 lg:pr-4 pt-8  sm:grid sm:grid-cols-2 md:grid md:grid-cols-4 lg:grid lg:grid-cols-5 gap-4 lg:gap-8 box-border'>
          {products.slice(0, visible).map(product => {
            return (
              <div key={product.id} className='rounded-2xl flex pt-0 flex-col w-11/12 bg-[#FFFFFF]  relative pt-8 h-300 rounded  sm:w-full md:w-1/1 lg:w-1/1 gap-8 shadow-sm bg-white hover:translate-y-1 duration-200 transition-all'>
                ...
                <Link to={`/product/${product.id}`}>
                  <div className='w-full h-100 flex items-center justify-center'>
                    <img src={product.img} alt='' className="h-full w-[60%]"></img>
                  </div>
