...
import { addtocart,removefavorite } from '../redux/data';
      ...
      <div className='items-center flex flex-col w-full'>
        <div className='grid w-full grid-cols-1 place-items-center sm:pl-4 sm:pr-4 lg:pl-4 lg:pr-4 pt-8  sm:grid sm:grid-cols-2 md:grid md:grid-cols-4 lg:grid lg:grid-cols-5 gap-4 lg:gap-8 box-border'>
          {favoriteproducts.map(product => {
            return (
              <div key={product.id} className='relative rounded-2xl flex flex-col w-11/12 bg-[#FFFFFF]  relative pt-8 h-300 rounded  sm:w-full md:w-1/1 lg:w-1/1 gap-8 shadow-sm bg-white hover:translate-y-1 duration-200 transition-all'>
                <CloseOutlined onClick={()=>{dispatch(removefavorite({id:product.id}))}} className='absolute right-2 top-2 font-bold text-black'/>
